...
- `scripts/ui-harness.ts` / `scripts/ui.test.ts` — headless acceptance suite for the UI contract (fake DOM + file-backed fetch over `api/wisdomtree/`): per-tab sort/filter persistence, 1-click search clear, exact selection scopes, race-free holdings loading, Watchlist dedupe and chunked rendering. Run with `bun test`.
...
- Verification before publishing: `bun test` (updater suite + UI contract acceptance suite) and `bun scripts/check-index.ts` (browser TypeScript transpile check).
